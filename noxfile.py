...
PIP_INSTALL_SILENT = False  # CI_RUN is False
...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=src/saltext/sap_xsa/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/sap_xsa/*")
            ...
            #    "--include=src/saltext/sap_xsa/*,tests/*"
            # )
        finally:
...
def gen_api_docs(session):
    ...
    session.run(
        ...
        "docs/ref/",
        "src/saltext",
        "src/saltext/sap_xsa/config/schemas",
    )
